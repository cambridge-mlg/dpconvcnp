    ...
    if dim == 1:
        ...
        for i in range(num_fig):
            if model is None:
                ...
                if hasattr(params, "no_dp") and params.no_dp:
                    _, model = dpsgp.utils.train_model(xc, yc, params)
                else:
                    _, model = dpsgp.utils.dp_train_model(xc, yc, params)
                ...
                with torch.no_grad():
                    ...
                    qf_pred_std = (qf_std.pow(2) + model.likelihood.noise.pow(2)).pow(
                        0.5
                    )
                    qf_pred = torch.distributions.Normal(qf_loc, qf_pred_std)

                    # model_nll = -model.likelihood.expected_log_prob(yt, qf).mean()
                    model_nll = -qf_pred.log_prob(yt).mean()
